	...
	return (
		<div style={{ border: "2px solid #ccc", padding: '20px' }}>
			<input 
				...
				onChange={e => setModules(prevState => {
			...
			<Button
			onClick={_ => setModules(prevState => {
				console.log(moduleIndex)
				console.log("Antes", prevState);
				const newState = [...prevState];
				
				newState[moduleIndex].lessons = [...newState[moduleIndex].lessons, { lesson_title: "" }]
			...
			{
				modules[moduleIndex].lessons.map(lesson => <Lesson />)
			}
		</div>
	)
}
...
export default function CourseMockado() {
	...
	return (
		<>
			<input placeholder="Titulo do curso" onChange={(e) => setCourse({courseTitle: e.target.value})}/>


			{modules.map( _ => <Module moduleIndex={modules.length} modules={modules} setModules={setModules} />)}

			<Button onClick={_ => setModules(prevState => [...prevState, {module_title: "", lessons: []}])}>Adicionar modulo +</Button>
